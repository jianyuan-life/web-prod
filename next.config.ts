...
const nextConfig: NextConfig = {
  ...
  env: {
    ...
    // v5.10.434 撤回:USE_PLAN_V4_* 在 workflow durable runtime 讀 process.env、
    // next.config env DefinePlugin inline 不及 workflow chunk(build 輸出實證仍 runtime 讀)。
    // 改在 workflows/generate-report/steps.ts module 頂層用 `??=` 設 runtime 預設(v5.10.435)。
  },
