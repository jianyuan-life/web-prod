...
//
// 維度:
//   1) sticky position 生效率(getComputedStyle position = sticky)
//   2) col-1 視覺可見率(scroll 後 col-1 仍在 viewport)
//   3) 內容無截斷率(white-space:nowrap 生效、文字完整)
//   4) header / body col-1 寬對齊率
//   5) hover 不蓋 sticky col-1 率(本輪近似測 z-index ≥ 5 且 bg 非透明)
...
(async () => {
  ...
  const ssDir = path.join(__dirname, 'v5_10_153_visual');
  ...
  for (const t of TOKENS) {
    for (const vp of VIEWPORTS) {
      ...
      const url = `https://jianyuan.life/report/${t.id}?cb=v153_${Date.now()}`;
      log(`\n=== ${t.name} ${vp.name} ===`);
      try {
        ...
        tableChecks.forEach((tc, tcIdx) => {
          ...
          const d1Pass = sc.th_isSticky && sc.td_isSticky;
          ...
          // d4 header/body align
          dimensions.d4_header_body_align.total++;
          if (tc.widthAligned) dimensions.d4_header_body_align.pass++;
          else failCases.push({
          ...
          // d5 z-index + bg opaque (hover not-overlap proxy)
          dimensions.d5_zindex_bg_correct.total++;
          const d5Pass = sc.th_zIndexValid && sc.td_zIndexValid && sc.th_bgOpaque && sc.td_bgOpaque;
          ...
          const status = d1Pass && tc.noTruncation && tc.widthAligned && d5Pass ? '✅' : '❌';
        ...
        // screenshot
        const ssPath = path.join(ssDir, `${t.name}_${vp.name}_v153.png`);
  ...
  fs.writeFileSync(
    path.join(__dirname, 'v5_10_153_eval_result.json'),
