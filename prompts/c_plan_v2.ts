// ============================================================
// C 方案（人生藍圖 $89）— 犀利版 Prompt v5.5(v3 重寫、整合 9 P0)
// v5.5 更新(2026-04-27、依 _audit_2026-04-27/FINAL_AUDIT_REPORT_2026-04-27.md):
//   ✅ P0-3 靈數 5 邏輯互斥(命運要求變化 vs 天生抗拒、明文點出撕裂)
//   ✅ P0-4 「—」hard schema check(任何欄位空白 = 不通過、必繞著寫)
//   ✅ P0-5/11 UI 白話化(< 5% 術語、清括號背書、刪佐證欄、流年術語白話)
//   ✅ P0-7 雙軌策略反向錯位修正(UI 白話 / PDF 專業互補、不可雙堆術語)
//   ✅ P0-8 7×15 交叉矩陣表強制
//   ✅ P0-9 ASCII 年度曲線 + 12 月 ★ 表
//   ✅ P0-10 TOP 3 → TOP 5 + 支持系統 + 信心度 ●●●●○
//   ✅ P0-12 PDF 古籍引用密度提升(每章 ≥ 1-2 條原文)
//   ✅ 起承轉合過渡語(章節間呼應、避免跳躍)
...
═══════════════════════════════════════════════════════════════
🆕 起承轉合過渡語規則(章節間呼應、避免跳躍)
═══════════════════════════════════════════════════════════════

每章開頭 1 句呼應前章結尾 + 每章結尾 1 句預告下章、讓整份報告有 narrative flow:
...
格式:不必每章都用「上一章...下一章...」公式、可以變化、但**呼應前章 + 預告下章的 narrative bridge 必須存在**。
...
🆕 v5.5 雙軌策略硬要求(P0-7):
1. 每章 UI 段(主文)術語密度 < 5%、白話結論優先
2. 每章末加 \`> 📚 命理深析(PDF 專屬深度版)\` 區塊、含 1-2 條古籍原文 + 命盤計算過程 + 派別考據
3. 章節間 narrative bridge:第二章末尾預告第三章、第三章開頭呼應第二章、第三章末尾預告第四章、第四章開頭呼應第三章
4. 任何欄位**禁寫「—」「N/A」「無資料」**(P0-4 hard schema)、缺值必繞著寫`
}
...
3. 每章末加 \`> 📚 命理深析(PDF 專屬深度版)\` 區塊、含 1-2 條古籍原文 + 命盤計算過程 + 派別考據
4. 章節間 narrative bridge:第五章開頭呼應第四章末尾、依序串接到第八章
5. 12 個月表格**必填白話建議**、**禁寫「注意健康」「保持平常心」**這類空話、必具體到「做什麼 / 不做什麼」
6. 任何欄位**禁寫「—」「N/A」「無資料」**(P0-4 hard schema)`
}
...
## 九、十五系統交叉驗證矩陣(P0-8、強制完整填寫、不可有 「—」)

> 一句話定調:「十五個系統都有自己的角度、但當它們指向同一個結論——你就該認真聽。」

用表格呈現 **7 個人生面向 × 15 個系統** 的交叉驗證結果。每格**必填** ★ 評級、不可空白、不可寫「—」、不可寫「...」:
...
🆕 v5.5 雙軌策略硬要求(P0-7)、本 Call 為集大成章節、必達:

1. **十五系統交叉矩陣完整填寫**(P0-8):7 面向 × 15 系統 × ★ 評級 + 共識度 🟢🟡🔴 全部填實值、不可有「...」「—」
2. **ASCII 年度曲線圖必繪**(P0-9):依範例格式、依實際命盤填高低
3. **12 月 ★ 評分表必填 12 行**(P0-9):每行整體 + 財運 + 事業 + 感情 + 健康 + 重點主題、每格白話
4. **TOP 5(非 TOP 3)+ 信心度 ●●●●○**(P0-10):優勢 5 條 + 風險 5 條、每條附支持系統 + ●●●●○
5. **幸運參數 8 行表必填**:每格依排盤實填、絕對禁寫「—」
6. **章節間 narrative bridge**:第九章末尾預告第十章、依序串接到第十七章
7. **每章末加 \`> 📚 命理深析(PDF 專屬深度版)\`**(P0-12)、含 1-2 條古籍原文 + 命盤計算過程 + 派別考據
8. **任何欄位禁寫「—」「N/A」「無資料」**(P0-4)、缺值必繞著寫

【不合格判定（嚴格、任一不過 = 報告失敗）】:
- 缺少任何一個章節(九-十七)→ 不合格
- 交叉矩陣有「...」placeholder 或不滿 7×15 = 105 格 → 不合格(P0-8)
- ASCII 年度曲線缺繪 → 不合格(P0-9)
- 12 月 ★ 表不滿 12 行或某月空白 → 不合格(P0-9)
- TOP 優勢 / 風險少於 5 條 → 不合格(P0-10)
- 任何欄位寫「—」「N/A」「無資料」「待補」→ 不合格(P0-4)
- 刻意練習少於 5 個 → 不合格
...
🆕 v5.5 雙軌策略提醒(P0-7):
- UI 段(主文)術語密度 < 5%、白話結論優先
- 每章末加 \`> 📚 命理深析(PDF 專屬深度版)\` 含古籍原文(1-2 條)+ 命盤計算 + 派別考據
- 任何欄位**絕對禁寫「—」「N/A」「無資料」「待補」**(P0-4 hard schema、缺值必繞著寫)
- 章節間 narrative bridge:每章開頭呼應前章結尾、每章結尾預告下章
- TOP 5(非 3)+ 信心度 ●●●●○、TOP 表格不要列「多系統佐證」欄`
