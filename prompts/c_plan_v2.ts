...
1. **命格封號**：
   - 🚨 **最高優先級硬規則**：封號**必須是恰好 4 個中文字**，且**必須完整複製排盤數據中提供的「人格封號」**（見前面客戶資料區的「人格封號」欄位）。
   - 封號**只能是以下 10 個之一**（對應十天干日主，排盤系統已根據你的八字日主自動選定）：
     * 甲木日主 → 「參天大樹」
     * 乙木日主 → 「柔韌藤蔓」
     * 丙火日主 → 「太陽之火」
     * 丁火日主 → 「燭火星光」
     ...
     * 壬水日主 → 「江河大海」
     * 癸水日主 → 「雨露甘霖」
   - ❌ **絕對禁止自創**：不得使用「一片被烈火燒過的…」「靜默的守望者」「暗夜中的利刃」等形容詞式或超過 4 字的封號。
   - ❌ **絕對禁止**：不得使用「命格就像…」這種句式作為封號，只能直接輸出 4 個中文字的封號。
   - ✅ **正確格式**：「**命格封號**：參天大樹」（同行冒號後直接寫 4 字封號，不加任何形容詞或修飾語）
...
`
  ...
  // 注入人格封號（供命格名片使用）
  // 🚨 命格封號硬規則：必須恰好 4 字，且必須從以下 10 個中選一個（依日主對應）
  ...
  if (dayMaster) {
    ...
    prompt += `\n════════════════════════════════════════\n`
    prompt += `【命格封號 — 硬規則，不可違反】\n`
    prompt += `════════════════════════════════════════\n`
    prompt += `日主：${dayMaster}\n`
    prompt += `人格封號：${persona.title}（恰好 4 個中文字）\n`
    prompt += `\n你在「一、命格名片」章節的「命格封號」欄位**必須**輸出「${persona.title}」這 4 個字，不可自創、不可加形容詞、不可用「命格就像」句式，只能直接寫 4 個字的封號。\n`
    prompt += `合法封號清單（只有以下 10 個，對應日主）：參天大樹(甲)、柔韌藤蔓(乙)、太陽之火(丙)、燭火星光(丁)、巍峨高山(戊)、沃土田園(己)、精鋼利刃(庚)、珠玉寶石(辛)、江河大海(壬)、雨露甘霖(癸)\n`
    prompt += `════════════════════════════════════════\n\n`
  }
  ...
  // v5.3.11：動態列出實際可用的系統清單，禁止 AI 編造沒提供的系統
  // 歷史 bug：原先寫死「15系統完整數據」導致 AI 即使只有 11 個系統也會自己補到 15 個，
  // 編造人類圖、Mahabote、數字能量學等沒算過的系統結果（5 LLM QA 抓到的命理幻覺根因）
  const activeSystems = analyses
    .filter(a => systemFilter.includes(a.system))
    .map(a => a.system)
  prompt += `\n## 排盤數據（以下 ${activeSystems.length} 個系統的排盤結果）\n`
  prompt += `\n【鐵則・禁止命理幻覺】\n`
  prompt += `1. **只能引用以下列出的 ${activeSystems.length} 個系統**：${activeSystems.join('、')}\n`
  prompt += `2. **嚴禁**引用或創造任何不在此清單的系統（例：清單沒有「人類圖」，整份報告禁止出現人類圖、能量中心、通道、閘門、類型等概念）\n`
  prompt += `3. **嚴禁**引用此系統 JSON 中未出現的欄位（例：紫微 JSON 沒有「自化」欄位，禁止生成「武曲自化祿/貪狼自化權」等論述）\n`
  prompt += `4. **嚴禁**編造奇門格局名稱/分數（例：若引擎沒有「玉女守門、丁奇歸巽」，禁止自創）\n`
  prompt += `5. 若某主題需要引用但系統未提供，明說「此項需要 XX 系統完整資料，建議另做 XX 專門分析」即可，**不要瞎編**\n`
  prompt += `6. **違反以上規則會被 5 LLM QA 直接降為 0 分，整份報告重寫**\n\n`
