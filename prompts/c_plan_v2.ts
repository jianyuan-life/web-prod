...
🔴🔴🔴 **章節順序鐵律(v5.7.43 根治、違反即重寫)**:
- 章節編號**必依 1-17 順序連續**(一、二、三...十七)、不跳號、不亂序
- 寫完章節 X、下一章必為 X+1、不可從「三」直接跳「九」
- ⚠️ **不準在章節標題加額外標記**(篇別 / 起承轉合 / 階段)、純編號 + 標題即可、前端會自動分篇
- 範例正確:`## 三、事業與天賦`(✅)
- 範例錯誤:`## 三、【承篇】事業與天賦`(❌、前端會 parser 錯亂)

🔴🔴🔴 **防重複鐵律(v5.7.43 根治、違反即重寫)**:
- 「人生速覽」三 callout(最大的天賦 / 最該注意的課題 / 2026 該做什麼)是序章 metadata、**只寫一次**(在 ## 你的人生速覽 區塊)
- 後續章節**禁止原文重複**這三 callout 的文字、可以引用主題但不抄整句
- 「你最大的天賦」這句話只在序章 callout 出現、章節內展開時改寫成不同句子

🔴🔴🔴 **廢話開頭鐵律(v5.7.43 根治)**:
- 章節開頭**禁止**:「接下來我們來看」「以下是」「現在我們」「在本章中」「上一章看到...」「讓我們來」
- 直接從結論第一句開始
...
⚠️ **大運硬規格(v5.7.43 根治、違反即重寫)**:
- **必須列完整 8 個大運**(共 80 年人生跨度、從起運歲開始、每柱 10 年)
- **🔴 年齡區間必寫整 10 年**:`0-9 歲` / `10-19 歲` / `20-29 歲` / `30-39 歲` / `40-49 歲` / `50-59 歲` / `60-69 歲` / `70-79 歲`
  - 起運若不在 0(如起運 3)、第 1 柱寫「3-9 歲」、第 2 柱仍寫「10-19 歲」、不寫「3-12 歲」「13-22 歲」這種跨歲
  - 嚴禁:「33-39 歲」「26-29 歲」「28-30 歲」這種非整 10 年區間(會讓前端時間軸跳號顯示錯亂)
- **不可只寫 5-6 個就停**(客戶看不到 70-80 歲展望)
- 漏寫任 1 柱 / 寫成非整 10 年區間 = quality gate fail
