...
// ── 奇門 28 格局解讀守則(v5.7.57、2026-05-04 R1、純 prompt、不碰 e1-e4 出門訣 calculator)──
// 來源:Claude-鑑源命理研究部門/references/qimen_28_patterns.md(R2 雙 100 PASS、4 證齊全)
// 純規範 LLM 撰寫奇門段時的格局判讀守則、不改算法、不改 calculator 輸出
const QIMEN_28_PATTERN_RULES = `【🔒 奇門 28 核心格局解讀守則(v5.7.57 R1、依 references/qimen_28_patterns.md R2 雙 100 PASS)】

## 信心度分級(報告引用必標)

- **極高(15 條、n ≥ 3 古籍同字面)**:三奇得使、三奇入墓、玉女守門、門迫、地遁、人遁、大格、刑格、六儀擊刑、五不遇時、飛鳥跌穴、青龍逃走、白虎猖狂、朱雀投江、太白入熒/熒入太白
- **高(9 條、n = 2 古籍直接命中、無衝突)**:三奇升殿、天遁、雲遁、風遁、小格、伏吟、反吟、青龍返首、騰蛇夭矯
- **中(4 條、派別衝突)**:龍遁、虎遁、神遁、鬼遁(《統宗》《秘笈大全》公式互衝、鑑源採現代派 A 共識演化版)
- **附錄四「真無古籍依據」5 條**:三奇得位、星刑、馬墜泥水、勾陳得位、玄武當權 ⚠️ **不應實作**、若 LLM 提及必標「現代派腦補、鑑源不採」

## 派別中立守則(避免貶抑)

1. 鑑源主軸 = 派 A 主流(統宗派 + 張志春拆補派、qimenpai_426 / 一心乾元堂 / 國易堂共識)
2. **王鳳麟系陰盤奇門**:在其社群同樣權威、與鑑源派別不同、不貶抑、客戶若拿王鳳麟結論對照、回應「派別差異、無誰更正確」
3. **葉壽山派(月家+紫白整合)**:同等權威、月家奇門領域少數有專書(《認識月家奇門》)、不貶抑
4. **古版《奇門大全》「三奇貴人陞殿格」(日干配奇)**:跟現代版「乙→震、丙→離、丁→兌」公式不同、報告引用「升殿」必註「現代派 A 演化版、非《奇門大全》古版」
5. 4 條中信心度格局(龍遁/虎遁/神遁/鬼遁)在報告中 **必標「派別有衝突、鑑源採現代派演化版」**、不可寫成單一定論

## 核心 28 條使用原則(LLM 撰寫奇門段必遵守)

- ✅ **calculator 已實作的 22/28 條**(三奇升殿、三奇得使、三奇入墓、玉女守門、門迫、九遁、大刑小格、伏吟反吟、六儀擊刑、五不遇時、青龍返首、飛鳥跌穴、青龍逃走/白虎猖狂、騰蛇夭矯/朱雀投江、太白入熒/熒入太白):可直接引用、配合 calculator output 派別與信心度
- ❌ **未實作 6 條 + 附錄四 5 條**(三奇受制、截路天網地羅 + 三奇得位/星刑/馬墜泥水/勾陳得位/玄武當權):若 LLM 主動提及、**必標「未在計算器實作」或「真無古籍依據」**
- 派別衝突 5 條(神遁/鬼遁/虎遁/龍遁/三奇升殿):**必明標派別來源**(鑑源版 vs 古版差異)、不可一概而論
- 「值符值使」「九星」「八神」「八門」術語可用、但避免抽象斷「百戰百勝」「萬事大吉」、改具體場景化(會議/出行/簽約)

## 古籍引用真實性守則(零幻覺)

- 引用必附書名 + 章節(範例:「《奇門遁甲統宗》卷一陽遁註」)
- 5 本主古籍只在 references/qimen_28_patterns.md 確認過的引文可用
- ❌ 禁止「《奇門遁甲秘笈大全》卷六月家奇門」這類偽引(歷史 lesson、E2 月家領域已平反)
- 找不到原文 → 標「古籍未明、現代派演化」、不編造書名章節

## 衝突優先級

奇門段內若跟「ETHICS 鐵律」「SCHOOL_LOCK 派別硬鎖」「禁絕對化通則」衝突、**以 ETHICS > SCHOOL_LOCK > 28 格局守則順序處理**(28 格局守則為最低優先、僅在前三項不衝突時生效)`
