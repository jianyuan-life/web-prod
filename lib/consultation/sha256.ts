...
/**
 * Synchronous, runtime-neutral SHA-256 for Workflow-reachable code.
 * It intentionally avoids node:crypto because Vercel Workflow functions are
 * compiled for a restricted runtime where Node built-ins are unavailable.
 */
export function sha256HexSync(input: string | Uint8Array): string {
  ...
  // JavaScript typed arrays cannot approach the 2^61-byte SHA-256 limit, so
  // two exact 32-bit words cover every input that can actually be allocated.
  const bitLengthHigh = Math.floor(source.length / 0x20000000)
  const bitLengthLow = (source.length << 3) >>> 0
  const lengthView = new DataView(padded.buffer, padded.byteOffset, padded.byteLength)
  lengthView.setUint32(padded.length - 8, bitLengthHigh, false)
  lengthView.setUint32(padded.length - 4, bitLengthLow, false)
