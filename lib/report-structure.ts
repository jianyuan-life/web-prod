...
function classifyC(title: string): ChapterPart {
  ...
  // ── v5.10.494:C v4/v6 章節名專屬分類(優先於下方 v2 通則)──
  // 根因(production 兒童實單 2b3cb069 目錄實測):v4 章名(原廠設定/核心競爭力與
  // 天賦學習路徑/精準診斷書/成長戰略推演…)在下方通則裡沒有對應規則、四散或
  // fallback 到 qi → 目錄出現「第一篇/第二篇/第四篇」缺第三篇、卻標「分 3 篇」。
  // 兒童版與成人版章名一起收斂,兩者都得到完整四篇。
  if (/原廠設定|底層邏輯/.test(t)) return 'qi'
  if (/核心競爭力|財富路徑|天賦學習路徑|學習路徑/.test(t)) return 'cheng'
  if (/人際磁場|同儕與家人/.test(t)) return 'cheng'
  if (/精準診斷|診斷書/.test(t)) return 'cheng'
  if (/戰略推演|五年戰略|成長戰略|關鍵節點/.test(t)) return 'zhuan'
  if (/開運與防禦清單|照護與教養清單|專屬開運|教養清單/.test(t)) return 'he'
