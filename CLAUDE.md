...
10. ~~**報告品質修復**~~ — ✅ 已完成（v5.2.0，Markdown表格/分數殘留清理）
11. ~~**Email 退訂機制**~~ — ✅ 已完成（v5.2.0）
12. ~~**章節折疊/展開 + 閱讀進度條**~~ — ✅ 已完成（v5.2.0）
13. ~~**後台路徑遷移 /jamie**~~ — ✅ 已完成（v5.2.0）
14. ~~**推薦積分系統**~~ — ✅ 已完成（v5.2.0，完整閉環含忠誠度後台）
15. **推薦積分系統實際測試** — 端對端測試註冊獎勵→首購獎勵→回購獎勵→積分折抵→贈與流程
16. **積分到期機制** — 目前積分永久有效，未來可考慮加入到期日

### 🟢 未來優化
17. **PDF 白底重設計** — 藝術部門方案已提交，部分已於 v4.5.24 實作（封面白底）
18. **PDF 附件加入 Email** — Resend 寄信時把 PDF base64 附上
19. **退款按鈕** — Admin 後台加 Stripe Refund API
20. **types.ts 死代碼清理** — FAMILY_EXTRA_PRICE G15:69 等
21. **推薦排行榜** — 前端展示推薦積分排名，激勵用戶分享
22. **積分商城** — 積分除折抵外的其他兌換方式
