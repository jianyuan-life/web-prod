          ...
          return (
            <div className="rounded-2xl px-6 py-5 mb-6" style={{
              ...
              {dayMaster && (() => {
                ...
                // v5.10.176 / 2026-05-11 修:frontend 自己算地支本氣、不依賴後端 wuxing_count(Fly.io 未 deploy)
                // 對應 Qwen audit L52「缺木、土、水」錯、實算何宣逸地支三戌一午、土 3 不缺
                // 算法:天干 1 個 + 地支本氣 1 個(每柱)
                const wxByZhi: Record<string, string> = {
                  '子': '水', '丑': '土', '寅': '木', '卯': '木',
                  '辰': '土', '巳': '火', '午': '火', '未': '土',
                  '申': '金', '酉': '金', '戌': '土', '亥': '水',
                }
                ...
                pillars.forEach(p => {
                  const ganWx = wxByGan[p[0] || '']
                  const zhiWx = wxByZhi[p[1] || '']
                  if (ganWx) wxCount[ganWx]++
                  if (zhiWx) wxCount[zhiWx]++
                })
